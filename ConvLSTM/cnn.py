# https://github.com/aserdega
# minor changes have been done by https://github.com/arianjafari
import torch
...
class ConvEncoder(nn.Module):
    def __init__(self, in_channel = 5, out_dim=64, norm = nn.BatchNorm2d):
        super(ConvEncoder, self).__init__()
        self.cnn_encoder = nn.Sequential(
...
class ConvDecoder(nn.Module):
    def __init__(self, b_size=32, out_channel = 1, inp_dim=64, norm = nn.BatchNorm2d):
        super(ConvDecoder, self).__init__()
        ...
        self.bn3    = norm(16)
        self.dconv4 = nn.ConvTranspose2d(16,out_channel,3,stride=1, padding=1)
...
class MCConvEncoder(nn.Module):
    def __init__(self, in_channel = 3, out_dim=256):
        super(MCConvEncoder, self).__init__()
        self.encoder = nn.Sequential(
                                nn.Conv2d(in_channel,64,5,stride=1,padding=2),
                                nn.BatchNorm2d(64),
                                nn.ELU(),
...
class MCConvDecoder(nn.Module):
    def __init__(self, b_size=32, out_channel = 3, inp_dim=256):
        ...
        self.decoder = nn.Sequential(
                                ...
                                nn.ELU(),
                                nn.Upsample(scale_factor=2),
                                nn.ConvTranspose2d(64,out_channel,5,stride=1,padding=2),
                                )
