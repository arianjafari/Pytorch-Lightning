...
class ConvLSTMCell(nn.Module):
    def __init__(self,
        ...
        self.conv = nn.Conv2d(in_channels=self.input_dim + self.hidden_dim,
                            ...
                            bias=self.bias) # 4 * to concatenate the operations then splitiing them
    ...
    def forward(self, input, prev_state):
        ...
        combined = torch.cat((input, h_prev), dim=1)  # concatenate along channel axis, B,C,H,W
    ...
    def init_hidden(self, batch_size):
        state = (torch.zeros(batch_size, self.hidden_dim,
                             self.height, self.width, device=self.conv.weight.device),
